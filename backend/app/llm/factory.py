...
def get_llm_client(
    ...
    if not effective_provider and model:
        ...
        if m_lower.startswith("omniroute/") or m_lower.startswith("omni/"):
    ...
    if effective_provider == "ollama":
        ...
        return OpenRouterLLMClient(model=target_model)
        ...
        return NvidiaLLMClient(model=target_model)

    elif effective_provider == "omniroute":
        target_model = model or settings.OMNIROUTE_MODEL
        return OmniRouteLLMClient(model=target_model)
