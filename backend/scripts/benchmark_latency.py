...
async def run_scenario(client: httpx.AsyncClient, base_url: str, scenario_name: str, question: str, token: str | None = None) -> Dict[str, Any]:
...
async def run_cold_start(base_url: str, token: str | None = None) -> Dict[str, Any]:
    print("\n--- Running Cold Start Measurement ---")
    print("Ensure the backend was JUST RESTARTED before running this.")
    async with httpx.AsyncClient() as client:
        result = await run_scenario(client, base_url, "Cold_Start", SCENARIOS["Simple"], token)
...
async def run_warm_measurements(base_url: str, iterations: int, token: str | None = None) -> List[Dict[str, Any]]:
    ...
    async with httpx.AsyncClient(timeout=120.0) as client:
        ...
        print("Pre-warming...")
        await run_scenario(client, base_url, "Warmup", "Hello", token)
        ...
        for name, question in SCENARIOS.items():
            ...
            for i in range(iterations):
                res = await run_scenario(client, base_url, name, question, token)
...
def main():
    ...
    parser.add_argument("--token", type=str, help="JWT Bearer token")
    ...
    if not token and args.email and args.password:
        import requests
        login_url = f"{args.host}/api/auth/login"
        print(f"Authenticating as {args.email}...")
        try:
            res = requests.post(login_url, json={"email": args.email, "password": args.password})
    ...
    if not token:
        print("Error: Authentication required for the chat API.")
        print("Please provide --token <JWT> OR --email <email> --password <password>.")
        sys.exit(1)
        
    all_results = []
    
    if args.cold:
        res = asyncio.run(run_cold_start(args.host, token))
        all_results.append(res)
        
    if args.warm:
        res_list = asyncio.run(run_warm_measurements(args.host, args.iter, token))
